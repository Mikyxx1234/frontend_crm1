...
export type CheckAgentStatusNodeData = {
  label: string;
  summary: string;
  stepIndex?: number;
  onDelete?: () => void;
...
type CheckAgentStatusRF = Node<CheckAgentStatusNodeData, "checkAgentStatus">;

/** Card binário (disponível / offline) — mesmo layout do horário comercial. */
export function CheckAgentStatusNode({ data, selected }: NodeProps<CheckAgentStatusRF>) {
  return (
    <FlowNodeShell
      selected={selected}
      type="fields"
    ...
    >
      ...
      />
      <FlowNodeHeader
        icon={<UserCheck className="size-3.5" strokeWidth={2.4} />}
        title={data.label}
      ...
      <div className="wf-node__outs">
        <div className="wf-node__out wf-node__out--ok fx-out fx-out--cond">
          ...
          <span className="fx-out__label flex-1">Disponível</span>
          <CustomHandle
            type="source"
        ...
        <div className="wf-node__out wf-node__out--err fx-out fx-out--error">
          ...
          <span className="fx-out__label flex-1">Offline</span>
          <CustomHandle
            type="source"
            position={Position.Right}
            id="false"
      ...
      <NodeInlineConfig
        selected={selected}
        stepType={data.stepType ?? "check_agent_status"}
        config={data.config}
