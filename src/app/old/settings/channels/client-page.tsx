...
const sidebarItems = [
  {
    id: "canais",
    label: "Canais",
    href: "/old/settings/channels" as const,
    icon: Radio,
  },
  { id: "equipe", label: "Equipe", href: "/old/settings" as const, icon: Users },
  { id: "geral", label: "Geral", href: "/old/settings" as const, icon: Settings2 },
] as const;
        ...
        <aside className="w-full shrink-0 lg:w-56">
          <nav className="space-y-1">
            {sidebarItems.map((item) => {
              const active =
                item.href === "/old/settings/channels"
                  ? pathname.startsWith("/old/settings/channels")
                  : pathname === item.href;
          ...
          <div className="flex flex-col gap-4 border-b border-border pb-6 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <nav className="mb-2 flex flex-wrap items-center gap-1 text-xs text-muted-foreground">
                <Link href="/old/settings" className="hover:text-foreground">
                  Configurações
                </Link>
