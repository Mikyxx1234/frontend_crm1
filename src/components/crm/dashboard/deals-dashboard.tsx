...
function FunnelSection({ funnel: funnelProp }: { funnel: DashboardData["funnel"] }) {
  ...
  return (
    <section className="flex flex-col rounded-[var(--radius-xl)] border border-[var(--glass-border)] bg-white p-4 shadow-sm">
          ...
          <HoverEffectGroup>
            {funnel.map((stage) => (
              <HoverEffectItem key={stage.id} itemKey={stage.id} className="shrink-0">
                <article className="flex h-full w-[252px] flex-col gap-3 rounded-[var(--radius-lg)] border border-[var(--glass-border)] bg-white p-3.5 shadow-sm">
