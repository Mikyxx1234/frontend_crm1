...
/** Limites oficiais Meta Cloud API — o cliente só vê até este tamanho. */
const META = {
  header: 60,
  footer: 60,
  body: 1024,
...
export function isWhatsAppPreviewable(stepType: string): boolean {
  return PREVIEWABLE.has(stepType)
}

type ActionKind = "reply" | "url" | "call" | "flow" | "copy"
type Choice = { id: string; title: string; description?: string; kind: ActionKind }
...
function asChoices(cfg: NodeConfig, outputs: Output[]): Choice[] {
...
function actionHint(kind: ActionKind) {
...
function WaText({ text, className }: { text: string; className?: string }) {
...
export function WhatsAppCustomerPreview({
  open,
  onOpenChange,
  stepType,
  config,
  ...
  const choices = useMemo<Choice[]>(() => {
    if (isQuestion || isProduct) return []
    if (stepType === "send_whatsapp_template" && tpl?.buttons.length) {
      return tpl.buttons.map((b, i) => ({
        id: `tpl_${i}`,
        title: clip(b.title, META.templateBtn),
        kind: b.kind,
      }))
    }
    const raw = asChoices(config, outputs).slice(0, 10)
    const listMode =
      stepType === "send_whatsapp_list" ||
      (stepType === "send_whatsapp_interactive" && raw.length > 3)
    return raw.map((c) => ({
      ...c,
      title: clip(c.title, listMode ? META.rowTitle : META.replyBtn),
      description: c.description ? clip(c.description, META.rowDesc) : undefined,
    }))
  }, [config, outputs, stepType, tpl, isQuestion, isProduct])
  ...
  const productBody = useMemo(() => {
    ...
    return [
      `*${name}*`,
      finalPrice != null ? `Valor: ${formatMoney(finalPrice)}` : null,
    ]
      .filter(Boolean)
      .join("\n")
  }, [config.content, config.productName, config.unitPrice, config.discountPercent])

  const body = clip(
    isProduct
      ? productBody
      : (stepType === "send_whatsapp_template"
          ? tpl?.bodyPreview || config.bodyPreview || cardPreview
          : undefined) ||
        ...
        cardPreview ||
        "",
    asList ? META.listBody : META.body,
  )
  ...
  const listButton = clip(
    (typeof config.button === "string" && config.button.trim()) || "Ver opções",
    META.listBtn,
  )
  const sheetTitle = header || listButton
  const sectionTitle = clip(typeof config.sectionTitle === "string" ? config.sectionTitle : "", META.section)
  ...
  const time = useMemo(() => nowLabel(), [open])
  ...
  }, [reply, listOpen, open, body, choices.length])

  const tap = (c: Choice) => {
    const note = actionHint(c.kind)
    if (note) {
      setHint(note)
      setReply(null)
      setListOpen(false)
  ...
  return (
    ...
    >
      ...
      >
        <DialogTitle className="sr-only">Preview WhatsApp — {bizName}</DialogTitle>
        <DialogDescription className="sr-only">
          Simulação do que o cliente vê no WhatsApp. Toque nos botões para testar.
        </DialogDescription>
        ...
        <p className="shrink-0 text-center text-[11px] text-white/80">
          {asList
            ? `Toque em “${listButton}” e escolha um item — é o que o cliente envia de volta`
            : choices.length
              ? "Toque num botão para ver a resposta do cliente"
              : "Assim o cliente recebe esta mensagem"}
        </p>
      </DialogContent>
    </Dialog>
  )
...
function ChoiceIcon({ kind }: { kind: ActionKind }) {
  const cls = "h-3 w-3"
...
function ReceivedBubble({ children }: { children: ReactNode }) {
  return (
...
function SentBubble({ children, time }: { children: ReactNode; time: string }) {
  return (
...
function MediaBlock({ type, fileName, src }: { type: string; fileName?: string; src?: string }) {
...
export function WhatsAppPreviewButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      type="button"
      aria-label="Ver como o cliente recebe no WhatsApp"
      title="Ver no WhatsApp"
      onClick={(e) => {
        e.stopPropagation()
        onClick()
      }}
      className="flex h-6 w-6 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-[#008069]"
    >
